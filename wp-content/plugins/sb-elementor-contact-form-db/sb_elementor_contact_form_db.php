...
define( 'SB_ELEM_CFD_DB_VERSION', '1.4' );
...
function sb_elem_cfd_submenu_cb() {
	...
	if ( $posts = get_posts( 'post_type=elementor_cf_db&posts_per_page=-1' ) ) {
		$forms = $forms2 = array();
		foreach ( $posts as $post ) {
			if ( $data = get_post_meta( $post->ID, 'sb_elem_cfd', true ) ) {
				$forms[ $data['extra']['submitted_on'] ] = $data['extra']['submitted_on'];
			}

			if ( $form_id = get_post_meta( $post->ID, 'sb_elem_cfd_form_id', true ) ) {
				$forms2[ $form_id ] = $form_id;
			}
		}
		...
		if ( isset( $_REQUEST['form_name'] ) ) {
			...
			$rows = sb_elem_cfd_get_export_rows( $_REQUEST['form_name'] );

			echo '<h3>CSV Content (by Submitted Page)</h3>';
			echo '<p>Please review the data below and press "Download CSV File" to start the download.</p>';
			...
			$rows = sb_elem_cfd_get_export_rows_by_form_id( $_REQUEST['form_id'] );
...
function sb_elem_cfd_get_export_rows( $form_name ) {
	$rows = array();
...
function sb_elem_cfd_get_export_rows_by_form_id( $form_id ) {
	$rows = array();
	...
	if ( $posts = get_posts( 'post_type=elementor_cf_db&posts_per_page=-1' ) ) {
		...
		foreach ( $posts as $post ) {
			...
			if ( $this_form_id = get_post_meta( $post->ID, 'sb_elem_cfd_form_id', true ) ) {
				if ( $this_form_id == $form_id ) {
					$data = get_post_meta( $post->ID, 'sb_elem_cfd', true );
